...
if ($project_id) {
    $stmt = $conn->prepare("SELECT name FROM projects WHERE id=? AND created_by=?");
...
if ($project_id) {
    $sql = "SELECT * FROM tasks WHERE project_id = $project_id ORDER BY due_date ASC";
} else {
  ...
  <table>
    ...
    <tbody>
      <?php if ($result->num_rows === 0): ?>
        <tr>
          <td colspan="7" style="text-align:center; padding:20px;">No tasks yet.</td>
        </tr>
      <?php else: ?>
